...
function insertHeightIcons() {
  ...
  cards.forEach(card => {
    ...
    if (accMatch) {
      ...
      acc.title = lang === 'fr' ? "Accompagné = enfant sous surveillance" : "Begeleid = kind onder toezicht";
    ...
    if (soloMatch) {
      ...
      solo.title = lang === 'fr' ? "Seul = accès autonome" : "Alleen = zelfstandig toegelaten";
    ...
    if (details.includes('new') && !card.querySelector('.badge-new')) {
      ...
      badge.textContent = lang === 'fr' ? 'NOUVEAU' : 'NIEUW';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Dynamisch scrollToTopBtn tooltip aanpassen
  const btn = document.getElementById("scrollToTopBtn");
  if (btn) {
    const lang = window.location.pathname.includes('/fr') ? 'fr' : 'nl';
    btn.title = lang === 'fr' ? "Retour haut de page" : "Terug naar boven";
  }
});
