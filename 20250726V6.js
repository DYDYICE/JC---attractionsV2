...
function insertHeightIcons() {
  ...
  cards.forEach(card => {
    ...
    if ((details.includes('new') || details.includes('nieuw')) && !card.querySelector('.badge-new')) {
      ...
      badge.textContent = window.location.pathname.includes('/fr') ? 'NOUVEAU' : 'NIEUW';
...
function setupHeightFilter() {
  ...
  selector.addEventListener('change', () => {
    ...
    document.querySelectorAll('.attraction-card').forEach(card => {
      ...
      const accRanges = [...details.matchAll(/(accompagné|begeleid)[: ]*([0-9]{1,3})[^\d]+([0-9]{1,3})cm/g)];
      ...
      const soloRanges = [...details.matchAll(/(seul|alleen)[: ]*([0-9]{1,3})[^\d]+([0-9]{1,3})cm/g)];
      ...
      const accPlus = details.match(/(accompagné|begeleid)[: ]*\+([0-9]{1,3})cm/i);
      if (accPlus && height >= parseInt(accPlus[2])) show = true;

      const soloPlus = details.match(/(seul|alleen)[: ]*\+([0-9]{1,3})cm/i);
      if (soloPlus && height >= parseInt(soloPlus[2])) show = true;
