...
function insertHeightIcons() {
  ...
  cards.forEach(card => {
    ...
    const accMatch = details.match(/(accompagné|begeleid)[: ]*([0-9+–\-to ]+cm)/i);
    if (accMatch) {
      const acc = document.createElement('div');
      acc.className = 'access-icon';
      acc.innerHTML = `<span class="emoji">👨‍👧</span><span style="color:#f57c00; font-weight:bold;">${accMatch[2]}</span>`;
      acc.title = "Accompagné = enfant sous surveillance";
      wrapper.appendChild(acc);
      added = true;
    }

    const soloMatch = details.match(/(seul|alleen)[: ]*([0-9+–\-to ]+cm)/i);
    if (soloMatch) {
      const solo = document.createElement('div');
      solo.className = 'access-icon';
      solo.innerHTML = `<span class="emoji">🧍</span><span style="color:#388e3c; font-weight:bold;">${soloMatch[2]}</span>`;
    ...
    if ((details.includes('new') || details.includes('nieuw')) && !card.querySelector('.badge-new')) {
...
document.querySelectorAll('.flip-card').forEach(card => {
  ...
  const name = card.dataset.name || '';
  const id = card.dataset.id || '';
  const lang = window.location.pathname.includes('/fr') ? 'fr' : 'nl';

  // Badge NEW / NOUVEAU
  if (details.toLowerCase().includes('new') && !card.querySelector('.badge-new')) {
    const badge = document.createElement('div');
    badge.className = 'badge-new';
    badge.textContent = lang === 'fr' ? 'NOUVEAU' : 'NEW';
    card.querySelector('.card-front').appendChild(badge);
  }

// Inject emoji icon group (🔍)
if (!card.querySelector('.click-icon')) {
  const iconGroup = document.createElement('div');
  iconGroup.className = 'click-icon';
  iconGroup.innerHTML = '🔍'; // Feel free to reorder
  card.querySelector('.card-front').appendChild(iconGroup);
}
  
  // Set 4x4 Adventure image dynamically by language
const img014 = document.getElementById('attraction-img-014');
if (img014) {
  img014.src = window.location.pathname.includes('/fr')
    ? 'https://www.jungle-city.be/site/files/images/attracties/attractie014fr.jpg'
    : 'https://www.jungle-city.be/site/files/images/attracties/attractie014nl.jpg';
}

  // Fill in card-back with multiline text
  const back = card.querySelector(`#back-text-${id}`);
  if (back) back.textContent = details;
